...
const StockListItem: React.FC<StockListItemProps> = ({
  ...
  return (
    <div 
      className={cn(
        "flex items-center justify-between border border-border rounded-lg hover:bg-muted/30 transition-colors",
        classes.container,
            ...
            {showIndicator && quote && (
              <div className="flex items-center space-x-1">
                {isPositive ? (
                  ...
                  <ArrowTrendingDownIcon className="h-3 w-3 text-danger-600" />
                )}
                <span className={cn(
                  "text-xs font-medium",
                  isPositive ? 'text-success-600' : 'text-danger-600'
                )}>
                  {quote.data_source === 'YAHOO_FINANCE_API' ? 'LIVE' : 'CACHED'}
                </span>
              </div>
            )}
          </div>
